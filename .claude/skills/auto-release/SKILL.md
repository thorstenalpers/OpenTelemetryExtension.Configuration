...
8. **Bump `<Version>`** in the csproj.

9. **Extend docs** — update `README.md` etc. for the changes/new dep versions.

10. **Release notes** — copy `templates/release-notes.md` to
...
11. **Commit** (no tag) — stage csproj(s), docs, release notes; message `release: v<version>`.

12. **Push & PR to master**
    - `git push -u origin release/v<version>`
    - `gh pr create --base master --head release/v<version> --title "release: v<version>" --body <notes>`

13. **Report** the PR link and remind: after merge, trigger **Deploy Nuget** manually
